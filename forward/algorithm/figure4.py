...
class GaussianMixture:
    ...
    def pdf(self, x):
        ...
        for k in range(self.K):
            pdf_sum += self.weights[k] * multivariate_normal.pdf(x, self.comp_mean[k], self.comp_cov[k])
        return pdf_sum

def display_gmm(gmm, n=400, ax_lim=-12, bx_lim=12, ay_lim=-12, by_lim=12, cmap="Reds", axis=None):
    if axis is None: fig, axis = plt.subplots()
    ...
    axis.contourf(X, Y, Z, levels=levels, cmap=cmap, norm=LogNorm(vmin=levels[0], vmax=levels[-1]), alpha=1)
    return axis
...
def entropic_partial_barycentric_interpolation(gmm_1, gmm_2, t, Lambda, reg):
    ...
    for i in range(gmm_1.K):
        for j in range(gmm_2.K):
            M[i, j] = cpp_engine.GaussianW2(gmm_1.comp_mean[i], gmm_2.comp_mean[j], 
                                           gmm_1.comp_cov[i], gmm_2.comp_cov[j])
    M /= np.max(M)
    W = cpp_engine.entropic_partial_ot(gmm_1.weights, gmm_2.weights, M, Lambda, reg, 1000, 1e-7)
    new_w, new_m, new_c = [], [], []
    ...
    for i in range(gmm_1.K):
        for j in range(gmm_2.K):
            if W[i, j] > 1e-5:
                ...
                new_w.append(W[i, j]); new_m.append(m_ij); new_c.append(c_ij)
...
def main():
    ...
    covs_B = 0.2 * np.array([[[1, -0.1], [-0.1, 1]], [[1, 0.2], [0.2, 1]], [[1, 0.0], [0.0, 1]]])
    ...
    for row_idx, (label, L, R) in enumerate(configs):
        ...
        for col_idx, t in enumerate(t_list):
            ...
            if row_idx == 0: ax.set_title(f"t = {t:.2f}")
            if col_idx == 0: ax.set_ylabel(label, fontsize=12, fontweight='bold')
